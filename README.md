...
- **Vercel Serverless Optimization**: Native ESM module resolution (`./db.js`), `/tmp` serverless SQLite database path support, global DOMMatrix/ImageData polyfills for PDF processing, dynamic lazy-loading for `pdf-parse`, and `vercel.json` file-tracing configurations.
